...
def main(prefix):
    ...
    assert regex.search(prefix) is None, (
        f"Prefix contains special chars: {prefix}"
        + ". Please remove special chars: [@!#$%^&*()<>?/\|}{~:]"
    )

    # Open and extract shelve file contents
    genes_to_filenames = shelve.open(f"{prefix}/genes_to_filenames.shelve")
    data = list(genes_to_filenames.iteritems())
    genes_to_filenames.close()

    # Remove existing shelve files
    for filename in glob.glob(f"{prefix}/genes_to_filenames.shelve.*"):
        os.remove(filename)
    ...
    for gene, filename in data:
        ...
        genes_to_filenames[gene] = f"./{prefix}{elements[1]}"
